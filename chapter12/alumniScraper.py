...
import pyinputplus, sys, time, logging, pprint, re, csv, openpyxl, threading, traceback
...
for location in locations:
    ...
    try:
        ...
        # for each .row.bizcard alumCardElem, i.e loop over alumnis loaded
        try: # in case get an error with one, move to the next:
            ...
            alum.setdefault("name", None)
            alum.setdefault("location", None)
            alum.setdefault("addresses", None)
            alum.setdefault("experience", None)
            alum.setdefault("primaryEmail", None)
            alum.setdefault("secondaryEmail", None)
            logging.info(f'something unknown went wrong with alum number: {index}, name: {alum["name"]}')
            traceback.print_exc()
        ...
        for index, location in enumerate(locationName):
            locationName[index] = location.title() # Capitalize all words eg. New York
